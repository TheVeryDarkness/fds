...
! $Date$
   ...
   CHARACTER(256) QFILE ! Quantity file
...
! **************************************************
   ...
   NFILES_EXIST = 0
!
   SEARCH_BNDF: DO I = 1, 500 ! Why 500?
...
!
   LOOP_FDS2AST: DO P = 1, COUNTER
      ...
      CALL MAKE_STRIP(OUTFILE)
      PRINT *, OUTFILE
      C_SIZE = C_S
      NX = N(P, 2)
      NY = N(P, 3)
      NZ = N(P, 4)
      ! Adjust the normal to an axis if it is near the axis
               ...
               READ_BLOOP: DO
                  ...
                  DO II = 1, NPATCH ! Read mesh boundary
                     ...
                     CASE (1)
                        ...
                        IF (BNDF_TYPE_CHOSEN == 'CENTERED') READ (12 + VAR, END=199) &
                           ((F(J, K, II), J=J1B(II) + 1, J2B(II) + 1), K=K1B(II) + 1, K2B(II) + 1)
                     CASE (2)
                        IF (BNDF_TYPE_CHOSEN == 'STAGGERED') READ (12 + VAR, END=199) &
                           ((F(I, K, II), I=I1B(II), I2B(II)), K=K1B(II), K2B(II))
                        IF (BNDF_TYPE_CHOSEN == 'CENTERED') READ (12 + VAR, END=199) &
                           ((F(I, K, II), I=I1B(II) + 1, I2B(II) + 1), K=K1B(II) + 1, K2B(II) + 1)
                     CASE (3)
                        IF (BNDF_TYPE_CHOSEN == 'STAGGERED') READ (12 + VAR, END=199) &
                           ((F(I, J, II), I=I1B(II), I2B(II)), J=J1B(II), J2B(II))
                        IF (BNDF_TYPE_CHOSEN == 'CENTERED') READ (12 + VAR, END=199) &
                           ((F(I, J, II), I=I1B(II) + 1, I2B(II) + 1), J=J1B(II) + 1, J2B(II) + 1)
                     END SELECT
                  END DO
...
SUBROUTINE SEARCH(STRING, LENGTH, LU, IERR)! Search 1 boundary quantity from the smokeview file
...
SUBROUTINE SEARCH2(STRING, LENGTH, STRING2, LENGTH2, LU, IERR, CHOICE)! Search 2 boundary quantity from the smokeview file
